...
export function HashSession() {
  useEffect(() => {
    ...
    // Also on hashchange, and that is not belt and braces — it is the case
    // that actually bites. Adding a fragment to a page the browser already has
    // open is a same-document navigation: no request, no remount, so a
    // mount-only effect never sees it. An operator with the console already
    // open who clicks their access link in that same tab would watch nothing
    // happen and the link burn. Reproduced on /admin/[campana] on 2026-07-27.
